  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="flex-1 px-4 py-5 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 flex flex-col gap-4">
          ...
          {form.isDriver && (
            <div className="flex flex-col gap-3 pt-3 border-t border-gray-50">
              {[
                { label: "車型",     key: "carModel", placeholder: "例：Toyota Camry" },
                { label: "車牌號碼", key: "carPlate", placeholder: "例：ABC-1234" },
              ].map(({ label, key, placeholder }) => (
                <div key={key}>
                  <label className="text-xs font-medium text-gray-500 mb-1 block">{label}</label>
                  <input
                    type="text"
                    placeholder={placeholder}
                    value={form[key as "carModel" | "carPlate"]}
                    onChange={(e) => set(key, e.target.value)}
                    className="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-700 placeholder-gray-400 outline-none border border-gray-100 focus:border-green-400 transition-colors"
                  />
                </div>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {["白色","銀色","黑色","灰色","藍色","紅色"].map((c) => (
                    <button key={c} onClick={() => set("carColor", c)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${form.carColor === c ? "bg-green-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                      {c}
                    </button>
                  ))}
                </div>
        ...
        <button
          onClick={handleSave}
          disabled={saving || saved || !form.name.trim() || !form.company.trim()}
